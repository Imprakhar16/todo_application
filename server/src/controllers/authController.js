...
import { resend_otp_html, send_otp_html } from "../utils/htmlCodes.js";
dotenv.config();
...
const createOTPtoken = (email, otp) => {
  return jwt.sign(
    { email, otp, exp: Math.floor(Date.now() / 1000) + 10 * 60 },
...
export const loginUser = async (req, res) => {
  try {
    ...
    if (!user)
      return res.status(400).json({ message: "Please Regiter Yourself" });
    if (!user.isVerified) {
      return res
        .status(400)
...
export const verifyUser = async (req, res) => {
  try {
    ...
    if (!otpToken || !otp) {
      return res
        .status(400)
        .json({ message: "OTP token and OTP are required" });
    }
  ...
  try {
    ...
    try {
      await sendOTP({
        ...
        body: resend_otp_html(user.name,otp),
      });
    } catch (emailError) {
